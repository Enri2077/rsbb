...
public:
	CoreGui(CoreSharedState& ss, CorePublicChannel& public_channel, CoreZoneManager& zone_manager) :
			ss_(ss), public_channel_(public_channel), zone_manager_(zone_manager), pub_(ss_.nh.advertise<roah_rsbb::CoreToGui>("/core/to_gui", 1, true)), pub_timer_(
					ss_.nh.createTimer(Duration(0.1), &CoreGui::transmit, this)), set_score_srv_(ss_.nh.advertiseService("/core/set_score", &CoreGui::set_score_callback, this)), manual_operation_complete_srv_(
					ss_.nh.advertiseService("/core/manual_operation_complete", &CoreGui::manual_operation_complete_callback, this)), omf_complete_srv_(
					ss_.nh.advertiseService("/core/omf_switches/complete", &CoreGui::omf_complete_callback, this)), omf_damaged_srv_(
					ss_.nh.advertiseService("/core/omf_switches/damaged", &CoreGui::omf_damaged_callback, this)), omf_button_srv_(
					ss_.nh.advertiseService("/core/omf_switches/button", &CoreGui::omf_button_callback, this)), connect_srv_(
					ss_.nh.advertiseService("/core/connect", &CoreGui::connect_callback, this)), disconnect_srv_(
					ss_.nh.advertiseService("/core/disconnect", &CoreGui::disconnect_callback, this)), start_srv_(
					ss_.nh.advertiseService("/core/start", &CoreGui::start_callback, this)), stop_srv_(ss_.nh.advertiseService("/core/stop", &CoreGui::stop_callback, this)), previous_srv_(
					ss_.nh.advertiseService("/core/previous", &CoreGui::previous_callback, this)), next_srv_(ss_.nh.advertiseService("/core/next", &CoreGui::next_callback, this)) {
		transmit();
	}
};
